...
def add_texts_and_titles():
    pr_list = session.query(press_release).filter(and_(press_release.pr_html != "",
        press_release.pr_html != "ERROR", press_release.pr_date != None,
        press_release.pr_text == None)).order_by(press_release.pr_date.desc()).all()
    print "Processing %s press releases" % (len(pr_list))
    counter = 0
    for pr in pr_list:
        counter += 1
