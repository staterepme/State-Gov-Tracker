...
def get_official_tweets(legid_to_get):
    ...
    for t in tweets:
        ...
        new_tweets.append({'tweet': t.tweet,
            'timestamp': t.timestamp.split(' ')[0],
            'url': url})
    return new_tweets
...
def MyRep(request):
    ...
    return render_to_response('profile.html', {"rep_name": rep_name, "rep_picture": rep_picture, "rep_bio": rep_bio,
        "rep_news": rep_news, "rep_twitter": rep_twitter,
        "rep_facebook": rep_facebook, "rep_votes": rep_votes})
...
def get_recent_votes(legid_to_get):
    ...
    for leg_vote in leg_votes:
        vote_type = LegisVotes.objects.get(vote_id=leg_vote.vote_id).type
